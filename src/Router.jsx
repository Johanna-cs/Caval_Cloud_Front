...
const Router = () => {
  ...
  const providerRiderProfile = useMemo(
    () => ({ riderProfile, setRiderProfile }),
    [riderProfile, setRiderProfile]
  );
  const providerResultsRiders = useMemo(
    () => ({ resultsRiders, setResultsRiders }),
    [resultsRiders, setResultsRiders]
  );

  const providerHorseProfile = useMemo(
    () => ({ horseProfile, setHorseProfile }),
    [horseProfile, setHorseProfile]
  );
  const providerResultsHorses = useMemo(
    () => ({ resultsHorses, setResultsHorses }),
    [resultsHorses, setResultsHorses]
  );

  const providerUserProfile = useMemo(() => ({ userProfile, setUserProfile }), [
    userProfile,
    setUserProfile,
  ]);
